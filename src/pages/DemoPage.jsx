...
const DemoPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    selectedProduct: '', // Product user wants a demo for
    message: '',
  });
  ...
  const handleChange = (e) => {
    ...
    setFormData(prev => ({ ...prev, [name]: value })); // Simplified as state is now text input
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      quantity: null, // Quantity is not relevant for demo requests
      // Conditional fields
    ...
    try {
      ...
      setSubmitMessage('Your demo request has been submitted successfully!');
      ...
      setSubmitMessage(`Failed to submit demo request: ${err.message}`);
      console.error('Demo submission error:', err);
    }
  };

  const countries = ["India", "USA", "Canada"]; // Example list
  // Removed the 'states' object as it's no longer needed for a text input
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans antialiased">
      ...
      <header className="bg-purple-700 text-white py-12 px-6 text-center">
        ...
        <p className="mt-2 text-lg">Fill in the below details to schedule a product demonstration</p>
      </header>
      ...
      <main className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-8 md:p-12 mt-8 md:mt-16 relative z-10">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Select Product for Demo */}
          <div className="md:col-span-2">
            <label htmlFor="selectedProduct" className="block text-gray-700 font-semibold mb-2">Product of Interest</label>
            {loadingProducts ? (
              <p className="text-gray-600">Loading products...</p>
            ) : productError ? (
              <p className="text-red-600">Error loading products: {productError}</p>
            ) : products.length === 0 ? (
              <p className="text-gray-600">No products available for demo.</p>
            ) : (
              <select
                id="selectedProduct"
          ...
          <div className="md:col-span-2 text-center mt-6">
            ...
            >
              {submitStatus === 'submitting' ? 'SUBMITTING...' : 'BOOK DEMO'}
            </button>
          </div>
          ...
          <div className="md:col-span-2 text-center mt-8">
            <p className="text-gray-700">Looking to order a product?{' '}
              <Link to="/order" className="text-purple-700 font-medium hover:underline">Go to Order Form</Link>
            </p>
          </div>
        </form>
      </main>
